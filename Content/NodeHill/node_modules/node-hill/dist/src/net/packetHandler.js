...
(function (ClientPacketType) {
    ...
    ClientPacketType[ClientPacketType["CustomUsername"] = 20] = "CustomUsername";
    ClientPacketType[ClientPacketType["CustomBodyColors"] = 21] = "CustomBodyColors";
...
function handlePacketType(type, socket, reader) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (type !== ClientPacketType.Authentication && type !== ClientPacketType.CustomUsername
            && type !== ClientPacketType.CustomBodyColors && !player)
            return;

        switch (type) {
            case ClientPacketType.CustomUsername: {
                try {
                    const customUsername = reader.readStringNT();
                    if (player) {
                        player.username = customUsername;
                    } else {
                        socket._customUsername = customUsername;
                    }
                } catch (err) {
                    return;
                }
            ...
            case ClientPacketType.CustomBodyColors: {
                try {
                    const hex = /^[0-9A-Fa-f]{6}$/;
                    const faceIdPattern = /^[a-zA-Z0-9_-]{0,64}$/;
                    const colors = {
                        headColor: reader.readStringNT(),
                        torsoColor: reader.readStringNT(),
                        leftArmColor: reader.readStringNT(),
                        rightArmColor: reader.readStringNT(),
                        leftLegColor: reader.readStringNT(),
                        rightLegColor: reader.readStringNT(),
                    };
                    const faceId = reader.readStringNT();
                    for (const key in colors) {
                        if (!hex.test(colors[key])) return;
                    }
                    if (!faceIdPattern.test(faceId)) return;
                    if (player) {
                        Game_1.default.emit("customBodyColors", player, colors);
                        if (faceId) player.setFace(faceId);
                    } else {
                        socket._customBodyColors = colors;
                        socket._customFaceId = faceId;
                    }
                } catch (err) {
                    return;
                }
            ...
            case ClientPacketType.Authentication: {
                ...
                const finalUsername = socket._customUsername || authResponse.username;
                
                Object.defineProperties(authUser, {
                    userId: { value: authResponse.userId },
                    username: { value: finalUsername, writable: true },
                ...
                if (socket._customBodyColors) {
                    Game_1.default.emit("customBodyColors", authUser, socket._customBodyColors);
                }
                if (socket._customFaceId) {
                    authUser.setFace(socket._customFaceId);
                }
                break;
            }
