...
//
// SECURITY (GHSA-489w-w794-jq94): `buf` is a bridge proxy of a HOST buffer, and the
// bridge no longer lets ownership be OBSERVED from here. It bounds a non-owning host
// view's backing store to the view's own bytes and reports that view's `byteOffset`
// as 0, so `buf.byteOffset === 0` and `buf.buffer.byteLength === buf.length` now read
// exactly the same for a pool-backed buffer as for an owning one. Reading `.buffer`
// at all is also expensive here — each read of a non-owning view's store is a copy.
//
// So decide by CONSTRUCTION instead, using Node's own pooling rule: `allocate()`
// serves a request out of the shared pool only when `size < (Buffer.poolSize >>> 1)`
// — note the `>>>`, which coerces, so a non-numeric `poolSize` still pools.
// Anything at or above that threshold is allocated standalone and already owns its
// store; anything below it is copied unconditionally by `depoolBuffer`. Over-copying
// is always safe (the copy owns its store) and costs at most half a pool; under-
// copying cannot happen while Node's rule holds, and if that rule ever changes the
// bridge-side bounding is the backstop.
function bufferOwnsExactBackingStore(buf) {
	let len, poolSize;
	try {
		len = buf.length;
		poolSize = LocalBuffer.poolSize;
	} catch (e) {
		return false;
	}
	if (typeof len !== 'number') return false;
	// SECURITY (GHSA-489w-w794-jq94): fail CLOSED on a non-numeric `poolSize`. Node's
	// `allocate()` compares `size < (Buffer.poolSize >>> 1)`, and `>>>` COERCES — a
	// string `'65536'` still pools — so "not a number" does not mean "pooling is
	// off". Answering false here just means the buffer gets copied, which is always
	// safe. `poolSize` 0 legitimately disables pooling and is handled by the
	// comparison itself (`len >= 0` is true, so nothing is copied).
	if (typeof poolSize !== 'number') return false;
	return len >= (poolSize >>> 1);
}
...
function bufferFrom(value, encodingOrOffset, length) {
	...
	// SECURITY (GHSA-489w-w794-jq94): a HOST-realm ArrayBuffer reaches the sandbox as
	// a bridge proxy (`hostBuf.buffer` is one), and the internal-slot brand test above
	// cannot recognize it — no sandbox-side getter has a slot to read on a Proxy. The
	// host still took the SHARING overload for it, so copying the result would silently
	// break the contract. Confirm it spoof-proof: a cheap ArrayBuffer shape gate
	// (byteLength, no length), then the decisive test that the result's own backing
	// store IS `value`. `result` is the host's own return value, so its store identity
	// comes from the bridge, not from anything sandbox code can forge — a look-alike
	// (e.g. `{byteLength: n, type: 'Buffer', data: [...]}`, which host resolves through
	// `fromArrayLike` to a pooled buffer) fails the identity test and is depooled.
	// A strict SUB-RANGE of a host-realm ArrayBuffer is depooled rather than shared:
	// its store is bounded, so it cannot equal `value`. See the behaviour note in
	// ATTACKS.md Category 41.
	if (
		value !== null &&
		typeof value === 'object' &&
