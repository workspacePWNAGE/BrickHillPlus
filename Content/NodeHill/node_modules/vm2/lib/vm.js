...
// The bootstrap sources are embedded string literals (lib/sources.js, generated
// by scripts/build-sources.js) rather than fs reads of `${__dirname}/...`, so
// single-file bundlers (Bun compile, esbuild, pkg) can ship vm2 without any
// configuration. They are compiled under a fixed virtual filename that keeps
// the shape of an absolute path: the GHSA-v27g / GHSA-x6m4 frame classifiers
// in setup-sandbox.js treat leading-`/` filenames as host frames and redact
// them, and bootstrap frames must stay redacted (see test/sources.js).
const BOOTSTRAP_FILENAME_PREFIX = '/vm2/lib/';

const bridgeScript = compileScript(
	BOOTSTRAP_FILENAME_PREFIX + 'bridge.js',
	`(function(global) {"use strict"; const exports = {};${sources['bridge.js']}\nreturn exports;})`,
);
...
const setupSandboxScript = compileScript(
	BOOTSTRAP_FILENAME_PREFIX + 'setup-sandbox.js',
	`let ${INTERNAL_STATE_NAME};(function(global, host, bridge, data, context) { ${sources['setup-sandbox.js']}\n})`,
);
...
exports.VM = VM;
exports.BOOTSTRAP_FILENAME_PREFIX = BOOTSTRAP_FILENAME_PREFIX;
